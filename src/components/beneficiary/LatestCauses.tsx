...
const CauseCard: React.FC<IStoryCardProps> = ({
  ...
  const progress =
    amount && raisedAmount ? Math.floor((raisedAmount / amount) * 100) : 0;
  ...
  const handleDonateBlood = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(newBloodDonationAction({ causeId: _id }))
          .unwrap()
          .then((response: any) => {
  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
        ...
        {requestType === "blood" && (
          <div className="space-y-3 text-sm text-gray-700">
            <p>
              ...
              <span className="text-red-700">{bloodGroup}</span>
            </p>
            <Button
        ...
        {requestType === "organ" && (
          <div className="space-y-3 text-sm text-gray-700">
            <p>
              ...
              <span className="text-green-700">{organType}</span>
            </p>
            <p>
      ...
      <div className="p-4 bg-gray-100 text-sm text-blue-700">
        Any concerns regarding this cause? <br />
        <br />
        <Button
          className="text-white bg-red-500"
          onClick={() => {
            navigate(`/concern/${beneficiary._id}`);
          }}
        >
          Report
        </Button>
...
const LatestCauses: React.FC = () => {
  ...
  const approvedStories = stories.filter(
    (story: any) => story.status === "approved"
  );
  const categorizedStories = {
    financial: approvedStories.filter(
      (s: any) => s.requestType === "financial"
    ),
  ...
  if (loading)
    return (
      <p className="text-center py-20 text-gray-600">
        Loading latest causes...
      </p>
    );
  if (error)
    return <p className="text-center py-20 text-red-500">Error: {error}</p>;

  const isEmpty = Object.values(categorizedStories).every(
    (arr) => arr.length === 0
  );
    ...
    <section className="py-20 bg-[#fcfcfc]">
      {isEmpty ? (
        <div className="text-center text-xl font-bold text-[#2c2520] mt-12">
          ...
          <p>
            We’ve successfully completed all the causes! Thank you for your
            support.
          </p>
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div>
              <span className="text-[#b8860b] text-2xl uppercase tracking-wider underline font-serif">
                Latest Causes
              </span>
