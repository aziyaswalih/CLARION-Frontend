  ...
  async (formData: volunteerDetails, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to submit volunteer details"
      );
    }
  }
);
...
const volunteerSlice = createSlice({
  ...
  reducers: {
    updateField: (
      state,
      action: PayloadAction<{ field: keyof VolunteerState; value: any }>
    ) => {
...
export const { updateField, resetForm } = volunteerSlice.actions;
