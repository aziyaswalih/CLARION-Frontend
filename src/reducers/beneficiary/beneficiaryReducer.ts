  ...
  async (formData: Partial<BeneficiaryState>, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to submit beneficiary details"
      );
    }
  }
);
...
const beneficiarySlice = createSlice({
  ...
  reducers: {
    updateField: (
      state,
      action: PayloadAction<{ field: keyof BeneficiaryState; value: any }>
    ) => {
    ...
    updateFamilyDetails: (
      state,
      action: PayloadAction<{ field: keyof FamilyDetails; value: string }>
    ) => {
    ...
    builder
      ...
      .addCase(submitBeneficiaryDetails.rejected, (state, action) => {
        ...
        state.submissionError =
          (action.payload as string) || "Submission failed";
      })
      ...
      .addCase(getBeneficiary.rejected, (state, action) => {
        state.submissionStatus = "failed";
        state.submissionError =
          (action.payload as string) || "Fetching profile failed";
      })
      ...
      .addCase(updateBeneficiaryProfile.fulfilled, (state, action) => {
        ...
        state.identificationNumber =
          data.updatedBeneficiary.identificationNumber;
      ...
      .addCase(updateBeneficiaryProfile.rejected, (state, action) => {
        state.submissionStatus = "failed";
        state.submissionError =
          (action.payload as string) || "Update profile failed";
      });
  },
});
