...
>("stories/createStory", async (formData, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error.response?.data?.message || "Failed to submit story"
    );
  }
});
...
export const updateReviewer = createAsyncThunk<
  ...
  { id: string },
  { rejectValue: string }
>("stories/updateReviewer", async ({ id }, { rejectWithValue }) => {
  try {
    const response = await userAxiosInstance.put(
      `/beneficiary/story/${id}/review`
    );
    return response.data.story;
  } catch (error: any) {
    return rejectWithValue(
      error.response?.data?.message || "Failed to update reviewer"
    );
  }
});
