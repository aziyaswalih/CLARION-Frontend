...
export const RecentStories = () => {
  ...
  const filteredStories = useMemo(() => {
    return stories
      ...
      .filter((story) =>
        selectedRequestType === "all" ? true : story.requestType === selectedRequestType
      )
      .sort((a, b) => {
  ...
  return (
    <div className="container mx-auto px-4">
      <h2 className="text-4xl font-sans text-center text-gray-800 mb-8 underline">Recent Stories</h2>
        ...
        {paginatedStories.map((story) => (
          ...
          >
            <img
              src={
                story.images && story.images.length > 0 && story.images[0]
                  ? `${import.meta.env.VITE_SOCKET_URL}/uploads/${story.images[0]}`
                  : defaultImage
              }
              alt={story.title}
              className="w-full h-56 object-cover"
            ...
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-4 line-clamp-3">Description: {story.description}</p>
              <p className="text-lg text-gray-600 mb-4">
                <span className="font-medium">Status:</span>{" "}
                {story.status.charAt(0).toUpperCase() + story.status.slice(1)}
              </p>
