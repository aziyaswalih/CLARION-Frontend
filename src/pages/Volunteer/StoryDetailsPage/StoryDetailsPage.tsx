...
const StoryDetailsPage: React.FC = () => {
  ...
  return (
    ...
    {showChat && (
      ...
      <VolunteerChatList  userId={story.beneficiary._id} />
    )}
    <div className="max-w-4xl mx-auto p-8 bg-white shadow-xl rounded-lg mt-10 space-y-6">
      <h1 className="text-3xl font-bold">{story.title}</h1>
         <div className="flex justify-end mt-8 ">
            <Button onClick={()=> setShowChat(true)}>Chat With Beneficiary</Button>
          ...
          {isEditing ? (
            ...
            <div className="space-y-4">
              ...
              {story.documents && story.documents.filter(img => img.trim() !== "").length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-700 mb-2">Documents:</h4>
                  <ul className="list-disc pl-6">
                    {story.documents.map((doc, idx) => (
                      <li key={idx}>
                        <a href={`${import.meta.env.VITE_SOCKET_URL}/uploads/${doc}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                          Document {idx + 1}
                        </a>
                      </li>
                    ))}
              ...
              {story.images && story.images.filter(img => img.trim() !== "").length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-700 mb-2">Images:</h4>
                  <div className="flex gap-4">
                    {story.images.map((img, idx) => (
                      <img key={idx} src={`${import.meta.env.VITE_SOCKET_URL}/uploads/${img}`} alt={`Story Image ${idx + 1}`} className="w-24 h-24 object-cover rounded-md shadow-md" />
                    ))}
                  </div>
                </div>
              )}
