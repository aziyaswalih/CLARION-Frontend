...
const DonationHistory: React.FC = () => {
  ...
  const handleClick = (requestType: 'blood' | 'organ' | 'financial',blood=null,organ=null,amount=null,date:Date) => {
  ...
  const filteredDonations = userDonations.filter((donation: any) => {
    const matchesSearch = donation.storyId.title.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesCategory = filterCategory === 'All' || donation.storyId.requestType === filterCategory;
  ...
  const categories = ['All', ...new Set(userDonations.map((d: any) => d.storyId.requestType))];

  return (
    <section className="py-10 px-6 max-w-5xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-10 text-[#2c2520] underline">My Donation History</h1>
      ...
      {isLoading ? (
        ...
        <p className="text-center text-lg text-[#2c2520]">No matching donations found.</p>
      ) : (
        <div className="space-y-4">
          {currentDonations.map((donation: any) => (
            <div key={donation._id} className="bg-white shadow rounded-lg p-4">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-500 capitalize">Category: {donation.storyId.requestType}</p>
                  <p className="text-sm text-gray-500">
                    Donated on: {new Date(donation.date).toLocaleDateString()}
                  </p>
