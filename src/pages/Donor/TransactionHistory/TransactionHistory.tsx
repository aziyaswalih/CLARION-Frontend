...
const TransactionHistory: React.FC = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (txn) =>
          ...
          txn.mode.toLowerCase().includes(term)
      );
    }

    if (filterStatus !== "all") {
      filtered = filtered.filter(
        (txn) => txn.status.toLowerCase() === filterStatus
      );
    }
  ...
  const totalPages = Math.ceil(
    filteredSortedTransactions.length / ITEMS_PER_PAGE
  );

  return (
    <section className="py-10 px-6 max-w-5xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-6 text-[#2c2520] underline">
        My Transactions
      </h1>
            ...
            <tbody>
              {paginated.map((txn) => (
                <tr key={txn._id} className="border-b hover:bg-gray-50">
                  ...
                  <td
                    className={`px-6 py-4 capitalize ${getPurposeColorClass(
                      txn.purpose
                    )}`}
                  >
                    {txn.purpose}
