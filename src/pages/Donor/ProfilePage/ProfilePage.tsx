...
const DonorProfilePage = () => {
    ...
    const { name, email, profilePic, phone, role } = useSelector((state: RootState) => state.donor.donorId);
    const { address, submissionStatus } = useSelector((state: RootState) => state.donor);
            ...
            <div className="max-w-2xl mx-auto p-8 bg-gray-200 shadow-xl rounded-2xl border border-gray-200 space-y-4">
                ...
                <div className="flex flex-col items-center space-y-4">
                    <div className="relative group">
                        ...
                        />
                        {isEditing && (
                            <label htmlFor="file-upload" className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-40 rounded-full opacity-0 group-hover:opacity-100 transition duration-300 cursor-pointer">
                                <Upload className="w-6 h-6 text-white" />
                            </label>
                        )}
                        {isEditing && <input id="file-upload" type="file" accept="image/*" onChange={handleProfilePic} className="hidden" />}
                    </div>
                </div>
