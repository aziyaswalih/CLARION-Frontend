...
const ChatList = () => {
  ...
  const [userDetails, setUserDetails] = useState<Map<string, UserStateTypes>>(
    new Map()
      ...
      dispatch(volunteer_get_MessagesvolunteerId(volunteer.id))
        ...
        .then(async (messages: Response_ChatsTypes[]) => {
          ...
          const userDetailsPromises = userIds.map(
            (userId) => dispatch(volunteer_get_UserDetails(userId)).unwrap() // Unwrap the payload to access user data
          );
    ...
    Close
          ...
          <div className="space-y-6">
            {chatList.map((chat) => {
              ...
              const userId =
                chat.sender === volunteer?.id ? chat.receiver : chat.sender;
              ...
              return (
                ...
                >
                  ...
                  <div className="ml-4 flex-1">
                    <h4 className="text-lg font-semibold text-black-200">
                      {user?.name}
                    </h4>
