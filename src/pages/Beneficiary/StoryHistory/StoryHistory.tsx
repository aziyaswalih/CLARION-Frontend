...
const StoryHistory: React.FC = () => {
  ...
  const [sortKey, setSortKey] = useState<"title" | "submittedAt">(
    "submittedAt"
  );
    ...
    <div className="min-h-screen bg-[#f4f9ff] flex flex-col">
      ...
      <section className="flex-grow px-6 py-12 bg-[#e9f5ff]">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-8 underline text-gray-800">
          ...
          <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
            ...
            <select
              value={sortKey}
              onChange={(e) =>
                setSortKey(e.target.value as "title" | "submittedAt")
              }
              className="px-3 py-2 border rounded-md"
            >
                    ...
                    <tr key={story._id} className="hover:bg-blue-50">
                      ...
                      <td className="px-4 py-2 border capitalize">
                        <span className={getStatusColorClass(story.status)}>
                          {story.status === "rejected" && story.reason
                            ? `${story.status} (${story.reason})`
                            : story.status}
                        </span>
                      </td>
          ...
          <DialogContent className="max-w-2xl bg-white rounded-lg shadow-xl p-6">
            <DialogHeader>
              ...
              <DialogDescription className="space-y-3 text-sm">
                ...
                {/* Documents */}
                {selectedStory.documents &&
                  selectedStory.documents.filter((img) => img.trim() !== "")
                    .length > 0 && (
                    <div className="text-left pt-2">
                ...
                {selectedStory.images &&
                  selectedStory.images.filter((img) => img.trim() !== "")
                    .length > 0 && (
                    <div className="text-left pt-2">
