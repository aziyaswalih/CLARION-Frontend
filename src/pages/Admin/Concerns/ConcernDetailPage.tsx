...
import {
  ...
  fetchConcernById,
  updateConcernStatus,
} from "../../../reducers/concern/concernReducer";
...
export default function AdminConcernDetailPage() {
  ...
  const { selectedConcern, loading, error, success } = useSelector(
  ...
  const handleUpdate = () => {
    if (!id || !status) return;
    dispatch(updateConcernStatus({ id, status, resolutionNote }))
      .unwrap()
      .then(() => {
        Swal.fire({
          icon: "success",
  ...
  return (
    <>
      ...
      <div className="p-12 max-w-2xl mx-auto  bg-white shadow rounded">
        ...
        {success && (
          <p className="text-green-600 mb-3">Updated successfully!</p>
        )}
        {error && <p className="text-red-600 mb-3">{error}</p>}

        <p>
          <strong>Subject:</strong> {selectedConcern?.subject}
        </p>
        <p>
          <strong>Description:</strong> {selectedConcern?.description}
        </p>
        <p>
          <strong>Reporter:</strong> {selectedConcern?.reporterId?.name}
        </p>
        <p>
          <strong>Reported Member:</strong>{" "}
          {selectedConcern?.reportedMemberId?.name}
        </p>
