...
const Editvolunteers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/admin/volunteers/edit/${id}`);
        const user = response.data.volunteer;
        setFormData({
          name: user.name || '',
          email: user.email || '',
          phone: user.phone || '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.put<{success:boolean, message: string }>(
        `http://localhost:5000/api/admin/volunteers/update/${id}`,
        formData
      );
          ...
          <div>
            ...
            <Select onValueChange={handleRoleChange} value={formData.role}>
              ...
              <SelectContent className="bg-white border-2 border-gray-300 shadow-lg">
                ...
                <SelectItem value="user">volunteers</SelectItem>
              </SelectContent>
            </Select>
